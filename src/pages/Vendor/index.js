...
const VendorList = (props) => {
    ...
    const handleDelete = async (vendorID) => {
        try {
            await axios.delete(`${URL}/delete/${vendorID}`, config);
            fetchData();
                            ...
                            data.map((item) => (
                                <tr key={item.vendorID}>
                                    ...
                                    <td className="action-icons"> {/* Apply action icons class */}
                                        <NavLink to={{ pathname: `update/${item.vendorID}` }} state={item} className="link-primary mx-2">
                                            <FontAwesomeIcon icon={faUserPen} />
                                        </NavLink>
                                        <div className="link-danger mx-2" onClick={() => handleDelete(item.vendorID)}>
